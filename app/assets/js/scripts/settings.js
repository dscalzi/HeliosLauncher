...
const settingsMusicVolumeRange = document.getElementById(
  "settingsMusicVolumeRange"
);
const settingsMusicVolumeLabel = document.getElementById(
  "settingsMusicVolumeLabel"
);
const allowMusicCb = document.getElementById("allowMusicCb");
...
settingsMusicVolumeRange.onchange = (e) => {
  const value = parseInt(e.target.getAttribute("value") * 100, 10);
  settingsMusicVolumeLabel.innerHTML = value + "%";
  updateMusic(ConfigManager.getAllowMusic(), e.target.getAttribute("value"));
};

allowMusicCb.onchange = (e) => {
  updateMusic(e.target.checked, ConfigManager.getMusicVolume());
};
...
function populateReleaseNotes() {
  $.ajax({
    url: "https://github.com/Kakumi/OPBSLauncher/releases.atom",
    success: (data) => {
